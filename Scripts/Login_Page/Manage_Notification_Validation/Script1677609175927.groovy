...
WebUI.callTestCase(findTestCase('Login_Page/Create_Notification_All_Field'), [('Notification_NO') : ''], FailureHandling.STOP_ON_FAILURE)
...
while (totalCount != rowCount) {
   ...
   // WebUI.callTestCase(findTestCase('Verify_Fields/Label_Display'), [:], FailureHandling.STOP_ON_FAILURE)
	...
	println("TotalCount = "+totalCount + " Field Value = " + GlobalVariable.Field_Name)
	...
	switch (GlobalVariable.Field_Type) {
		...
		case 'AttachDisplay':
			...
			WebUI.callTestCase(findTestCase('Verify_Fields/Attach_Display'), [:], FailureHandling.STOP_ON_FAILURE)
		...
		case 'Tab':
			...
			if (GlobalVariable.Field_Name.equals("Attachments"))
			{
				
				GlobalVariable.index = 2
